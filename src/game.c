...
void moveDown()
{
  ...
  if (moveWillCauseCollision(playerX, playerY+1)) {
...
void showDialogBox(char *s1, char *s2)
{
  ...
  // position 7,9 is top left of dialog

  textcolor(COLOR_LIGHTBLUE);
  gotoxy(11,11); printf("%s", s1);

  textcolor(COLOR_LIGHTGREEN);
  gotoxy(15,13); printf("%s", s2);

  textcolor(COLOR_LIGHTBLUE);
  
  if (livesRemaining) {	// restart level
  
    gotoxy(15,15); printf("%s", "get ready!");
    sleep(4);
  
  } else {	// game over
  
    gotoxy(8,15); printf("%s", "z to continue, x to exit");
  
    while(1) {
    
      // OR together reads from keyboard, joy stick 1 and joy stick 2
      joy = (joy_read(0) | joy_read(1) | joy_read(2));

      if (joy & JOY_BTN_1_MASK) {	// keyboard x , button B on joystick
        while (kbhit()) cgetc();	// empty keyboard buffer
        exitToBasic();
      } else if (joy &  JOY_BTN_2_MASK) {	// keyboard z, button A on joystick
        break;
      }
    }	// end while
  }
...
void readJoystickAndMove()
{
    ...
    if ((joy & JOY_MOVE_MASK) == 0 || stoppedByCollision) {
      if (fallingSave) {  // kill the player
        playDropBoulderSound();
        showMessage("you have been squashed dead");
        dead=true;
        seconds=0;
      }
    }
    if (fallingSave!=0) fallingSave=0;
    if (stoppedByCollision) stoppedByCollision=false;

}

void exitToBasic()
{
  // change VERA config so that basic screen looks normal
